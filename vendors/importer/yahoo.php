...
class yahoo extends Importer {
	...
	function run($login, $password) {
		...
		curl_setopt($this->curl, CURLOPT_URL,"http://mlogin.yahoo.com/w/login/user?ssl=false&.ts=1309962781&.intl=US&.lang=en");
		...
		$params .= "__submit=Sign In&id=".$this->login."&password=".$this->password;			
		
		curl_setopt($this->curl, CURLOPT_URL,"http://mlogin.yahoo.com/w/login/auth?.ts=1309963620&.intl=US&.lang=en");
		curl_setopt($this->curl, CURLOPT_RETURNTRANSFER,1);
		curl_setopt($this->curl, CURLOPT_FOLLOWLOCATION, 1);
		curl_setopt($this->curl, CURLOPT_SSL_VERIFYPEER, 0);
