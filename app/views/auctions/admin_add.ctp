		...
		<div class="col-md-12">
			...
			if($this->requestAction('/settings/enabled/testing_mode') || $dummySet) :
				...
				<div id="autobidBlock" style="display: none">
					...
					<?php if($this->requestAction('/settings/get/randomize_autobids') || $dummySet) : ?>
			...
			if($this->requestAction('/settings/get/bids_back_random') || $dummySet) {
				...
				<div id="bidsBackBlock" style="display: none">
					<?php echo $form->input('bids_back_total', array('label' => __('Total Bids Back', true),'class' => 'form-control', 'after' => __(' Set to 0 to give the users total bids back', true),'style' => 'margin-top:-2px;')); ?>
				</div>
				<?php
			}
			...
			<?php echo $form->input('buy_it_reduction_amount', array('label' => __('', true),'class' => 'form-control', 'after' => __(' Amount to deduct per user bid.', true),'style' => 'margin-top:2px;')); ?>
			</div>
			<?php
			// End Change
			
		?>
		<?php //echo $form->end(__('Add Auction >>', true), array('class' => 'col-md-12', 'id' => 'auction-add-end'));?>
		<div class="clearfix"></div>
		<div class="submit">
			<input type="submit" value="Add Auction >>" style="margin-left:auto;padding:10px;text-align: center;">
		</div>
		</div>
	</fieldset>
