...
class CaptchaHelper extends AppHelper{
	...
	protected function writeCaptcha($controller){
		echo $this->Html->image($this->Html->url(array('controller'=>$controller,'action'=>'captcha'),true),array('id'=>'cakecaptcha', 'style' => 'margin-left:20px'));
		echo "<br/>";
		?>
		
		
        <span class="captchaText">Please enter the code above</span>
		<a href="#captcha" onclick="document.getElementById('cakecaptcha').src='<?php echo $this->Html->url(array('controller'=>$controller,'action'=>'captcha')); ?>?'+Math.random();
    document.getElementById('captcha-form').focus();"
    id="change-image" class="captchaText">Reload Image</a>
		<br/>
        <?php
            echo $this->Form->input('cakecaptcha',array('id'=>'captcha-form','name'=>'data[cakecaptcha][captcha]','label'=>'', 'class'=>'form-control', 'div' => array('class' => 'col-sm-12')));
