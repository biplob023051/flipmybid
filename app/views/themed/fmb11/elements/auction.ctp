<div class="col-md-4">
<div class="col-md-12 auction-item" key="<?php echo $auction['Auction']['id'];?>" id="auction_<?php echo $auction['Auction']['id'];?>">
	<h3><?php echo $html->link(/*$text->truncate($auction['Product']['title'],35)*/ $auction['Product']['title'], array('controller' => 'auctions', 'action' => 'view', $auction['Auction']['id']));?></h3>
	...
	<?php if(!empty($auction['Product']['rrp'])) : ?>
		<p><?php __('RRP');?>: <?php echo $number->currency($auction['Product']['rrp'], $appConfigurations['currency']); ?></p>
	<?php endif; ?>
...
<!-- this is for the auction view on mobile! -->
	<span style="display:none;">	
<?php if(!empty($bidHistories)):?>
                          <?php foreach($bidHistories as $bid):?>

                                    <?php // echo $time->niceShort($bid['Bid']['created']);?>
                                    <?php // echo $bid['User']['username'];?>
                                    <?php echo $bid['Bid']['description'];?>
                            <?php endforeach;?>
                    <?php elseif(!empty($auction['Winner']['id'])): ?>
                        <p class="bidhistory"><?php __('Not available.');?></p>
                    <?php elseif(!empty($auction['Auction']['closed'])): ?>
                        <p class="bidhistory"><?php __('No bids were placed.');?></p>
                    <?php else: ?>
                        <p class="bidhistory"><?php __('No bids have been placed yet.');?></p>
                    <?php endif;?>	
...
<!-- this is for the auction view on mobile! -->
	...
	<?php if(empty($auction['Auction']['closed']) && empty($auction['Auction']['isFuture'])):?>
		<?php if($session->check('Auth.User')):?>
			<a href="/bid.php?id=<?php echo $auction['Auction']['id']; ?>" class="bidbutton bid-button-link" title="<?php echo $auction['Auction']['id']; ?>">
			<?php $bidDebit = $this->requestAction('/settings/get/bid_debit/'.$auction['Auction']['id']); if(!empty($bidDebit)) : ?><?php echo $bidDebit; ?><?php else : ?>1<?php endif; ?>x bid<?php if(!empty($bidDebit) && $bidDebit > 1) :?>s<?php endif; ?></a>
