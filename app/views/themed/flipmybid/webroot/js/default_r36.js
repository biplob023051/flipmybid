$(document).ready(function(){
    ...
    $('.auction-item').each(function(){
        ...
        if($('#' + auctionId + ' .countdown').length){
            ...
            auctionObjects[auctionId]['buyitnow']                = $('#' + auctionId + ' .buyitnow');
    ...
    if(auctions){
        ...
        setInterval(function(){
            $.ajax({
                ...
                dataType: 'json',
                type: 'POST',
                data: auctionForGetBuyItPrice,
                success: function(data){

                    $.each(data, function(i, item){
                        var price = '&pound;' + item.BuyIt.price + '';
                        auctionObjects[item.Auction.element]['buyitnow'].html('<a class="remind" href="http://flipmybid.com/buy_it_packages/add/' + item.BuyIt.user_id + '/' + item.Auction.auction_id + '">Buy it for ' + price + '</a>');
                    });
    ...
    $('.bid-button-link').click(function(e){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if(data.Auction.balance){
                    bidBalance.html(data.Auction.balance);
                }
