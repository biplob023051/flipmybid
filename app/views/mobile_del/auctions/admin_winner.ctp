...
<?php if(!empty($auction['Auction']['payment'])) : ?>
	Owing: <?php echo $number->currency(($auction['Auction']['price'] + $auction['Product']['delivery_cost']) * -1, $appConfigurations['currency']); ?><br />
	<strong>Payment Method: <?php echo $auction['Auction']['payment']; ?></strong>

	<?php if(strtolower($auction['Auction']['payment']) == 'paypal') : ?>
		(<?php echo $auction['Winner']['paypal']; ?>)
	<?php elseif(strtolower($auction['Auction']['payment']) == 'amazon voucher') : ?>
		(<?php echo $auction['Winner']['amazon']; ?>)
	<?php elseif(strtolower($auction['Auction']['payment']) == 'bank transfer') : ?>
		(<?php echo $auction['Winner']['bank_transfer']; ?>)
	<?php endif; ?>
<?php else : ?>
...
<p>Username: <?php echo $auction['Winner']['username']; ?></p>

<?php if(!empty($auction['Product']['cash'])) : ?>
	<p><strong>PayPal Email Address: <?php echo $auction['Winner']['paypal']; ?></strong></p>
<?php endif; ?>
...
<dl class="editForm">
	...
	<?php if(!empty($auction['Auction']['charity'])) : ?>
		<dt><label><?php __('Charity');?>:</label></dt>
		<dd><?php echo $form->checkbox('resolved_charity', array('div' => false, 'label' => false, 'error' => false)); ?> Resolved Charity</dd>
	<?php endif; ?>
