...
class InvitesController extends AppController{
    ...
    function index() {
		if(!$this->Setting->get('user_invite_message')) {
		...
		if (!empty($this->data)) {
            ...
            $data['subject']  = __('Check out this website', true).' - '.$this->appConfigurations['name'];
            ...
            $this->data['Invite']['message'] = $this->Setting->get('user_invite_message');
           	...
           	$this->data['Invite']['message'] = str_replace('SENDER', $this->Auth->user('username'), $this->data['Invite']['message']);
           	$this->data['Invite']['message'] = str_replace('\n', "\n", $this->data['Invite']['message']);
        }

        $this->set('title_for_layout', __('Invite My Friends', true));
	}
}
?>